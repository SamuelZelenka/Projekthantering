...
public class Dragable : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(transform.position, Vector3.down, out hit, Mathf.Infinity))
        {
            ...
            if (hit.collider.gameObject.tag == "Table" && ifPlaced == true)
            {
                if (hit.collider.gameObject.transform.position.z < this.gameObject.transform.position.z && Input.GetMouseButton(0) == false)
                {
                    gameObject.transform.position = new Vector3(0.16f, 2.7f, 4.08f + 1.50f);
                }
                else if (hit.collider.gameObject.transform.position.z > this.gameObject.transform.position.z && Input.GetMouseButton(0) == false)
                {
                    gameObject.transform.position = new Vector3(0.16f, 2.7f, 4.08f - 1.50f);
                }
            }
